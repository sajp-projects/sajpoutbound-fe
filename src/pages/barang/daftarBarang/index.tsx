import { useProducts, useDeleteProduct } from "@/hooks/barang";
...
export default function DaftarBarang() {
    ...
    onError: (error) => {
      try {
        ...
        if (error.message && error.message.includes("Forbidden")) {
          showForbiddenAlert("Akses Ditolak", "Anda tidak memiliki akses untuk menghapus barang ini.");
        } else {
          const errorObj = JSON.parse(error.message);
          showErrorAlert("Gagal Menghapus Barang", errorObj.message || "Terjadi kesalahan saat menghapus barang");
        }
      } catch {
        showErrorAlert("Gagal Menghapus Barang", error.message || "Terjadi kesalahan saat menghapus barang");
      }
    },
  });
  ...
  const handleDeleteProduct = (id: string, name: string) => {
    showDeleteConfirmationAlert("Barang", `Apakah Anda yakin ingin menghapus barang "${name}"?`).then((result) => {
  ...
  const getProductActions = (product: Product) => [
    ...
    { type: ActionType.LOG },
    {
      ...
      isLoading: deleteProductMutation.isPending && deleteProductMutation.variables?.id === product.id,
  ...
  return (
    <div className="flex flex-col min-h-full w-full space-y-4 sm:space-y-6 px-2 sm:px-4 md:px-0">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-2 sm:gap-3 w-full">
        <h1 className="text-xl sm:text-2xl font-bold text-gray-900">Daftar Barang</h1>
      ...
      <div className="bg-white rounded-lg shadow p-3 sm:p-4 md:p-6 overflow-hidden w-full">
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-4 sm:mb-6 gap-3 w-full">
          <div>
            <h2 className="text-lg sm:text-xl font-semibold text-gray-900">Barang</h2>
            <p className="text-xs sm:text-sm text-gray-500">Manajemen data barang dan stok</p>
          </div>
        ...
        <div className="mb-4 sm:mb-6 w-full">
          <SearchInput placeholder="Cari barang..." className="w-full sm:max-w-md" />
        </div>

        {isLoading ? (
          <LoadingState text="Memuat data barang..." />
        ) : isError ? (
          <ErrorState title="Gagal memuat data barang" message={productError instanceof Error ? productError.message : "Terjadi kesalahan pada server"} onRetry={() => refetch()} />
        ) : (
          <div className="w-full">
                ...
                <Table>
                  <TableHeader>
                    <TableRow className="bg-gray-50 border-b border-gray-200">
                      ...
                      <TableHead className="w-[15%] py-3 px-3 text-left font-semibold text-gray-700 text-sm">SKU</TableHead>
                      <TableHead className="w-[20%] py-3 px-3 text-left font-semibold text-gray-700 text-sm">Nama</TableHead>
                      <TableHead className="w-[15%] py-3 px-3 text-left font-semibold text-gray-700 text-sm">Harga</TableHead>
                      <TableHead className="w-[10%] py-3 px-3 text-left font-semibold text-gray-700 text-sm">Stok</TableHead>
                      <TableHead className="w-[15%] py-3 px-3 text-left font-semibold text-gray-700 text-sm hidden md:table-cell">Gudang</TableHead>
                  ...
                  <TableBody>
                    {products.length === 0 ? (
                      ...
                      products.map((barang, idx) => (
                        <TableRow key={barang.id} className={cn(idx % 2 === 0 ? "bg-white" : "bg-gray-50", "border-b border-gray-200 last:border-b-0")}>
                          ...
                          <TableCell className="py-2.5 px-3 font-medium text-blue-600 text-sm">
                            <div className="truncate max-w-full" title={barang.sku}>
                              {barang.sku}
                            </div>
                          </TableCell>
                          <TableCell className="py-2.5 px-3 text-blue-600 font-medium text-sm">
                            <Link to={`/barang/${barang.id}`} className="hover:underline">
                              {barang.name}
                            </Link>
                          </TableCell>
                          <TableCell className="py-2.5 px-3 text-gray-700 text-sm">{formatRupiah(Number(barang.price))}</TableCell>
                          <TableCell className="py-2.5 px-3 text-gray-700 text-sm">{barang.quantity}</TableCell>
                          <TableCell className="py-2.5 px-3 text-gray-500 text-xs lg:text-sm hidden md:table-cell">
                            {barang.warehouse ? (
                              <Link to={`/gudang/${barang.warehouse.id}`} className="text-blue-600 hover:underline">
                          ...
                          <TableCell className="py-2.5 px-3 text-gray-500 text-xs lg:text-sm hidden md:table-cell">{formatDate(barang.createdAt)}</TableCell>
                          <TableCell className="py-2.5 px-3">
                            <div className="flex justify-center items-center">
                              <ActionButtons actions={getProductActions(barang)} entityId={barang.id} basePath="/barang" />
                            </div>
                          </TableCell>
                        </TableRow>
                      ))
            ...
            <div className="sm:hidden space-y-3 w-full">
              {products.length === 0 ? (
                ...
                products.map((barang) => (
                  <div key={barang.id} className="border border-gray-200 rounded-lg bg-white overflow-hidden shadow-sm w-full">
                    <div className="p-3 w-full">
                      <div className="flex justify-between items-start mb-2 w-full">
                        <div className="max-w-[65%]">
                          <h3 className="font-medium text-blue-600 break-words text-sm">{barang.name}</h3>
                          <p className="text-xs text-gray-600 mt-1">SKU: {barang.sku}</p>
                          <div className="flex flex-col mt-1">
                            <span className="text-xs font-medium text-gray-700">Harga: {formatRupiah(Number(barang.price))}</span>
                            <span className="text-xs font-medium text-gray-700">Stok: {barang.quantity}</span>
                          </div>
                        </div>
                      </div>
                      ...
                      <div className="text-xs text-gray-500 space-y-0.5 mb-2">
                        <p>
                          Dibuat: <span className="font-medium">{formatDateShort(barang.createdAt)}</span>
                        </p>
                        {barang.warehouse && (
                          <p>
                            Gudang:{" "}
