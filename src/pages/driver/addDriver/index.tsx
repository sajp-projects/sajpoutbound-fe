...
import { useCreateDriver } from "@/hooks/driver";
import { cn } from "@/lib/utils";
import { DriverCreateInput } from "@/types/driver";
...
export default function TambahSupir() {
  ...
  const createDriverMutation = useCreateDriver({
    onSuccess: () => {
      showSuccessAlert("Sukses!", "Supir berhasil ditambahkan").then(() => {
        navigate("/supir");
      });
    },
    onError: (error) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const submitData: DriverCreateInput = {
      name: formData.name.trim(),
    };

    showConfirmationAlert(
      "Konfirmasi",
      "Apakah Anda yakin ingin menambahkan supir ini?"
    ).then((result) => {
  ...
  return (
    <div className="px-4 space-y-6 sm:px-0">
      <div className="flex items-center">
        <h1 className="text-2xl font-bold text-gray-900">Tambah Supir</h1>
      </div>

      <Card className="border border-gray-200 rounded-lg shadow-sm">
        <CardHeader>
          <CardTitle>Form Supir Baru</CardTitle>
          <CardDescription>
            Isi data supir yang akan ditambahkan ke sistem
          </CardDescription>
        </CardHeader>
