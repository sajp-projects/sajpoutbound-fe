import { useDeleteDriver, useDrivers } from "@/hooks/driver";
...
export default function DaftarSupir() {
  ...
  const hasDriverCreateAccess = hasPermission(
    permissions,
    PERMISSION.RESOURCES.DRIVER,
    PERMISSION.ACTIONS.CREATE
  );

  const hasDriverUpdateAccess = hasPermission(
    permissions,
    PERMISSION.RESOURCES.DRIVER,
    PERMISSION.ACTIONS.UPDATE
  );

  const hasDriverDeleteAccess = hasPermission(
    permissions,
    PERMISSION.RESOURCES.DRIVER,
  ...
  const deleteDriverMutation = useDeleteDriver({
    onSuccess: () => {
      showSuccessAlert("Sukses!", "Supir berhasil dihapus");
      refetch();
    },
    onError: (error) => {
      showErrorAlert(
        "Gagal Menghapus Supir",
        error.message || "Terjadi kesalahan saat menghapus supir"
      );
    },
  });
  ...
  const handleDeleteDriver = (id: string, name: string) => {
    showDeleteConfirmationAlert(
      "Supir",
      `Apakah Anda yakin ingin menghapus supir "${name}"?`
    ).then((result) => {
  ...
  const getDriverActions = (driver: Driver) => {
    const actions: ActionConfig[] = [];

    if (hasDriverUpdateAccess) {
      actions.push({ type: ActionType.EDIT });
    }

    if (hasDriverDeleteAccess) {
      actions.push({
        type: ActionType.DELETE,
        onClick: () => handleDeleteDriver(driver.id, driver.name),
        isLoading:
          deleteDriverMutation.isPending &&
          deleteDriverMutation.variables?.id === driver.id,
  ...
  return (
    <div className="flex flex-col w-full min-h-full px-2 space-y-4 sm:space-y-6 sm:px-4 md:px-0">
      ...
      <div className="w-full p-3 overflow-hidden bg-white rounded-lg shadow sm:p-4 md:p-6">
        <div className="flex flex-col items-start justify-between w-full gap-3 mb-4 sm:flex-row sm:items-center sm:mb-6">
          <div>
            ...
            <p className="text-xs text-gray-500 sm:text-sm">
              Manajemen data supir kendaraan
            </p>
          </div>
        </div>
        ...
        {isLoading ? (
          <LoadingState text="Memuat data supir..." />
        ) : isError ? (
          <ErrorState
            title="Gagal memuat data supir"
            message={
                ...
                <Table>
                  <TableHeader>
                    <TableRow className="border-b border-gray-200 bg-gray-50">
                      ...
                      <TableHead className="w-[20%] py-3 px-3 text-left font-semibold text-gray-700 text-sm hidden md:table-cell">
                        Tgl. Dibuat
                      </TableHead>
                      <TableHead className="w-[20%] py-3 px-3 text-left font-semibold text-gray-700 text-sm hidden md:table-cell">
                        Tgl. Diperbarui
                    ...
                    {drivers.length === 0 ? (
                      <TableRow>
                        <TableCell colSpan={5} className="h-24 text-center">
                          <EmptyState title="Tidak ada data supir yang ditemukan" />
                        </TableCell>
                      </TableRow>
                    ) : (
                      drivers.map((driver, idx) => (
                        ...
                        >
                          ...
                          <TableCell className="py-2.5 px-3 font-medium text-blue-600 text-sm">
                            <div className="wrap-text" title={driver.name}>
                              {driver.name}
                            </div>
                          </TableCell>
                      ...
                      <div className="text-xs text-gray-500 space-y-0.5 mb-2">
                        <p>
                          ...
                          <span className="font-medium">
                            {formatDateShort(driver.createdAt)}
                          </span>
                        </p>
                        <p>
                          Diperbarui:{" "}
                          <span className="font-medium">
                            {formatDateShort(driver.updatedAt)}
                          </span>
                        </p>
                      </div>
